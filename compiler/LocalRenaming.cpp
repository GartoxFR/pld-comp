...
void LocalRenamingVisitor::visit(ir::Function& function) {
    ...
    for (uint32_t i = 1; i <= function.argCount(); i++) {
        usageVisitor.usedLocal().insert(Local {i});
    }
    ...
    for (auto& usedLocal : usageVisitor.usedLocal()) {
        ...
        Local newLocal = [&]() {
            if (oldInfo.name()) {
                return function.newLocal(std::move(oldInfo.name().value()));
            } else {
                return function.newLocal();
            }
        }();
...
void LocalRenamingVisitor::visit(ir::ConditionalJump& jump) {
    tryRename(jump.condition());
}
...
void LocalUsageVisitor::visit(ir::ConditionalJump& jump) {
    setUsed(jump.condition());
}
